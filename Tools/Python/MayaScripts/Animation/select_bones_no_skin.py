...
def select_bones_without_skin_weights():
    """
    Selects all joints/bones that don't have any skin weights
    across all skinClusters in the scene.
    Handles multiple meshes bound to one rig.
    """
    ...
    if not all_skin_clusters:
        ...
        print(f"Selected {len(all_joints)} joints (no skinClusters in scene)")
        return
    ...
    # Select the joints without weights
    if joints_without_weights:
        cmds.select(joints_without_weights, replace=True)
        print(f"Selected {len(joints_without_weights)} joints without skin weights:")
        for joint in joints_without_weights:
            print(f"  - {joint}")
    else:
        cmds.select(clear=True)
        print("All joints have skin weights.")
